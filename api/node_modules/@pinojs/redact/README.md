...
// Output: {"headers":{"cookie":"[***REMOVED***]","authorization":"[***REMOVED***]","x-forwarded-for":"192.168.1.1"},"user":{"name":"john","password":"[***REMOVED***]"}}
...
- **censor** `any` (optional, default: `'[***REMOVED***]'`): The value to replace sensitive data with. Can be a static value or function.
...
const redact = slowRedact({
  paths: ['password'],
  censor: (value, path) => `***REMOVED***:${path}`
})
```
...
console.log(result.secret) // '[***REMOVED***]'
...
|--------|----------|----------------|
| Default (redact) | Replaces values with censor | `{"password":"[***REMOVED***]"}` |
