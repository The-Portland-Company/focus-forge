...
test('basic path redaction', () => {
  ...
  assert.strictEqual(parsed.headers.cookie, '[***REMOVED***]')
...
test('multiple paths redaction', () => {
  ...
  // Result redacted
  const parsed = JSON.parse(result)
  assert.strictEqual(parsed.user.password, '[***REMOVED***]')
  assert.strictEqual(parsed.session.token, '[***REMOVED***]')
...
test('serialize: false returns object with restore method', () => {
  ...
  assert.strictEqual(result.secret, '[***REMOVED***]')
...
test('bracket notation paths', () => {
  ...
  const parsed = JSON.parse(result)
  assert.strictEqual(parsed['weird-key']['another-weird'], '[***REMOVED***]')
...
test('array paths', () => {
  ...
  const parsed = JSON.parse(result)
  assert.strictEqual(parsed.users[0].password, '[***REMOVED***]')
  assert.strictEqual(parsed.users[1].password, '[***REMOVED***]')
...
test('wildcard at end of path', () => {
  ...
  const parsed = JSON.parse(result)
  assert.strictEqual(parsed.secrets.key1, '[***REMOVED***]')
  assert.strictEqual(parsed.secrets.key2, '[***REMOVED***]')
...
test('wildcard with arrays', () => {
  ...
  const parsed = JSON.parse(result)
  assert.strictEqual(parsed.items[0], '[***REMOVED***]')
  assert.strictEqual(parsed.items[1], '[***REMOVED***]')
  assert.strictEqual(parsed.items[2], '[***REMOVED***]')
})
...
test('intermediate wildcard', () => {
  ...
  const parsed = JSON.parse(result)
  assert.strictEqual(parsed.users.user1.password, '[***REMOVED***]')
  assert.strictEqual(parsed.users.user2.password, '[***REMOVED***]')
})
...
test('censor function', () => {
  ...
  const redact = slowRedact({
    paths: ['secret'],
    censor: (value, path) => `***REMOVED***:${path.join('.')}`
  })
  const result = redact(obj)

  const parsed = JSON.parse(result)
  assert.strictEqual(parsed.secret, '***REMOVED***:secret')
})
...
test('custom serialize function', () => {
  ...
  assert.strictEqual(parsed.secret, '[***REMOVED***]')
  assert.strictEqual(parsed.public, 'data')
})
...
test('nested paths', () => {
  ...
  const parsed = JSON.parse(result)
  assert.strictEqual(parsed.level1.level2.level3.secret, '[***REMOVED***]')
})
...
test('null and undefined handling', () => {
  ...
  const parsed = JSON.parse(result)
  assert.strictEqual(parsed.nullValue, '[***REMOVED***]')
  assert.strictEqual(parsed.nested.nullValue, '[***REMOVED***]')
})
...
test('type safety: accessing properties on primitive values should not throw', () => {
  ...
  assert.doesNotThrow(() => {
    ...
    assert.strictEqual(parsed2.data.item2.nested, '[***REMOVED***]') // Object property redacted
  })
...
test('empty string bracket notation path', () => {
  ...
  // Result should have redacted path
  const parsed = JSON.parse(result)
  assert.strictEqual(parsed[''].c, '[***REMOVED***]')
})
...
test('censor function not called for non-existent nested paths', () => {
  ...
  assert.strictEqual(result3.headers.authorization, '[***REMOVED***]')
})
