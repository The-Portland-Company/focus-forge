...
test('three consecutive wildcards: *.*.*.password (4 levels deep)', () => {
  ...
  assert.strictEqual(parsed.nested.deep.auth.password, '[***REMOVED***]', '4-level password SHOULD be redacted')
})
...
test('four consecutive wildcards: *.*.*.*.password (5 levels deep)', () => {
  ...
  assert.strictEqual(parsed.config.user.auth.settings.password, '[***REMOVED***]', '5-level password SHOULD be redacted')
})
...
test('five consecutive wildcards: *.*.*.*.*.password (6 levels deep)', () => {
  ...
  assert.strictEqual(parsed.data.reqConfig.data.credentials.settings.password, '[***REMOVED***]', '6-level password SHOULD be redacted')
})
...
test('three wildcards with censor function receives correct values', () => {
  ...
  assert.strictEqual(parsed.nested.deep.auth.password, '[***REMOVED***]')
})
...
test('three wildcards with multiple matches', () => {
  ...
  assert.strictEqual(parsed.api1.v1.auth.token, '[***REMOVED***]')
  assert.strictEqual(parsed.api2.v2.auth.token, '[***REMOVED***]')
  assert.strictEqual(parsed.api3.v1.auth.token, '[***REMOVED***]')
})
...
test('mixed: two and three wildcards in same redactor', () => {
  ...
  assert.strictEqual(parsed.user.auth.password, '[***REMOVED***]', '3-level should be redacted by *.*.password')
  assert.strictEqual(parsed.config.deep.auth.password, '[***REMOVED***]', '4-level should be redacted by *.*.*.password')
})
...
test('three wildcards with arrays', () => {
  ...
  assert.strictEqual(parsed.users[0].auth.password, '[***REMOVED***]')
  assert.strictEqual(parsed.users[1].auth.password, '[***REMOVED***]')
})
...
test('four wildcards with authorization header (real-world case)', () => {
  ...
  assert.strictEqual(parsed.requests.api1.config.headers.authorization, '[***REMOVED***]')
  assert.strictEqual(parsed.requests.api2.config.headers.authorization, '[***REMOVED***]')
})
