...
test('prototype pollution: __proto__ path should not pollute Object prototype', () => {
  ...
  assert.strictEqual(result.__proto__.isAdmin, '[***REMOVED***]')
})
...
test('prototype pollution: constructor.prototype path should not pollute', () => {
  ...
  assert.strictEqual(result.constructor.prototype.isAdmin, '[***REMOVED***]')
})
...
test('prototype pollution: nested __proto__ should not pollute', () => {
  ...
  assert.strictEqual(result.user.settings.__proto__.isAdmin, '[***REMOVED***]')
})
...
test('prototype pollution: wildcard with __proto__ should not pollute', () => {
  ...
  assert.strictEqual(result.users.user1, '[***REMOVED***]')
  assert.strictEqual(result.users.user2, '[***REMOVED***]')
  ...
  if (Object.prototype.hasOwnProperty.call(obj.users, '__proto__')) {
    assert.strictEqual(result.users.__proto__, '[***REMOVED***]')
  }
})
...
test('prototype pollution: verify prototype chain is preserved', () => {
  ...
  assert.strictEqual(result.data, '[***REMOVED***]')
...
test('prototype pollution: deep nested prototype properties should not pollute', () => {
  ...
  assert.strictEqual(result.level1.level2.level3.__proto__.isAdmin, '[***REMOVED***]')
  assert.strictEqual(result.level1.level2.level3.constructor.prototype.isEvil, '[***REMOVED***]')
})
