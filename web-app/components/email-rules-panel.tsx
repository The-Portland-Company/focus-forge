...
import { Bot, Loader2, Plus, ShieldCheck, Sparkles, Wand2 } from "lucide-react";
import {
  Select,
  SelectContent,
  SelectItem,
...
type EmailRulesPanelProps = {
  ...
  className?: string;
  /**
   * Opt-in (spam-review modal only): renders a quick "Not Spam" action on rows
   * for active rules that push mail INTO spam. Omitting this leaves the default
   * row render untouched for every other usage of this shared panel.
   */
  onMarkRuleNotSpam?: (rule: EmailRule) => void;
  /** Rule id currently being marked not-spam, for the per-row busy state. */
  markingRuleNotSpamId?: string | null;
};

// Actions that force matching mail into spam/quarantine/deletion. A "Not Spam"
// affordance is only meaningful for rules carrying one of these.
const SPAM_FORCING_RULE_ACTIONS = new Set(["spam", "quarantine", "always_delete"]);

function ruleForcesSpam(rule: EmailRule) {
  return rule.actions.some((action) =>
    SPAM_FORCING_RULE_ACTIONS.has(action.type),
  );
}
        ...
        <div className="min-w-0 space-y-3">
          ...
          {localRules.length === 0 ? (
            ...
            localRules.map((rule) => {
              ...
              // Only the spam-review modal opts in, and only active spam-forcing
              // rules get the "Not Spam" affordance; everything else renders the
              // exact original row untouched.
              const canMarkNotSpam =
                Boolean(onMarkRuleNotSpam) &&
                rule.isActive &&
                ruleForcesSpam(rule);
              ...
              const isMarking = markingRuleNotSpamId === rule.id;

              return (
                <div
                  key={rule.id}
                  data-email-rule-row={rule.id}
                  className="flex items-stretch rounded-xl border border-zinc-800 bg-zinc-900/50 transition-colors hover:border-zinc-700 hover:bg-zinc-900/80"
                >
                  <button
                    type="button"
                    onClick={() => startEditingRule(rule)}
                    className="min-w-0 flex-1 rounded-l-xl px-4 py-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-500/40"
                  >
                    {rowBody}
                  </button>
                  <div className="flex items-center border-l border-zinc-800 px-3">
                    <button
                      type="button"
                      disabled={isMarking}
                      onClick={() => onMarkRuleNotSpam?.(rule)}
                      aria-label={`Mark rule ${rule.name} as not spam — stop treating matching mail as spam`}
