...
import { deleteReplyDraft, updateReplyDraft } from "@/lib/email-inbox/server";
...
export async function DELETE(
  ...
  try {
    const params = await props.params;
    const result = await deleteReplyDraft({
      userId: auth.user.id,
      draftId: params.id,
    });
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "Failed to delete reply draft",
      },
      { status: 400 },
    );
  }
