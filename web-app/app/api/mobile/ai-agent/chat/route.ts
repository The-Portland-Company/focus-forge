...
// POST /api/mobile/ai-agent/chat
// Bearer-token mirror of /api/ai-agent/chat for the native iOS app. Same body,
// same response shape, same session persistence — only the auth differs: this
// accepts a Supabase access token (what the iOS app sends) or a PAT via
// verifyMobileAccessTokenOrPat, instead of a browser cookie session.
export async function POST(request: NextRequest) {
  try {
    ...
    // projectId is optional: present when the user is on a project screen,
    // absent when the assistant is used globally (Today/Tasks views).
    let project: { id: string; name: string } | null = null;
    if (projectId) {
      project = await getAuthorizedProject(admin, userId, projectId);
      if (!project) {
    ...
    if (agentSessionId) {
      ...
      if (!existing || existing.user_id !== userId || !sameScope) {
        agentSessionId = null;
      }
    }
    ...
    // extra user-visible context turn appended after the user message.
    if (vision?.context) {
