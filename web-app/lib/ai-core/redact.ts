...
export function redactSensitiveMemoryText(input: string): string {
  ...
  // OpenAI secret keys
  out = out.replace(/sk-[A-Za-z0-9_-]{20,}/g, "[***REMOVED***]")

  // Supabase PATs
  out = out.replace(/sbp_[a-f0-9]{40,}/g, "[***REMOVED***]")

  // GitHub tokens (ghs_ = server, ghp_ = personal)
  out = out.replace(/gh[ps]_[A-Za-z0-9]{30,}/g, "[***REMOVED***]")

  // Bearer tokens in headers / inline
  out = out.replace(/Bearer\s+[A-Za-z0-9\-._~+/]+=*/gi, "Bearer [***REMOVED***]")

  // password: value  or  password=value
  out = out.replace(/(password\s*[:=]\s*)\S+/gi, "$1[***REMOVED***]")
  ...
  // SSN  ddd-dd-dddd
  out = out.replace(/\b\d{3}-\d{2}-\d{4}\b/g, "[***REMOVED***]")
